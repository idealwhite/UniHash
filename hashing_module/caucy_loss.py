...
def caucy_loss(u, v, label_u, label_v, hashbit = 48,gamma=1.0, q_lambda=1.0,normed=True):
    ...
    cauchy_mask = cauchy#torch.masked_select(cauchy, Variable(mask).cuda())
    s_mask =s #torch.masked_select(s, Variable(mask).cuda())
    balance_p_mask = balance_param#torch.masked_select(balance_param, Variable(mask).cuda())


    all_loss = - s_mask * \
        torch.log(cauchy_mask) - (1.0 - s_mask) * \
        torch.log(1.0 - cauchy_mask)
    #cos_loss = torch.mean(all_loss)
