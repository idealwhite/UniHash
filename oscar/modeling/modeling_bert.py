...
class BertImgModel(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,#101,2045,2024
        ...
        return outputs
def normal_label(labels,class_number=24,max_length=10):
    all_size = labels.shape[0]
    final_label = []
    all_index = torch.arange(0,class_number)
    for i in range(all_size):
        this_label = torch.zeros((max_length))
        index =  all_index[labels[i]>0]
        index = index[0:max_length]
        index = index+1#增加一项padding的维度
        this_label[0:index.shape[0]] = index
        final_label.append(this_label)
    final_label = torch.stack(final_label).long()
    return final_label

# class HashingformerAll(nn.Module):
#     """
#     Modified from BertForSequenceClassification to support oscar training.
#     """
#     def __init__(self,oscar_checkpoint_path,config):
#         super(HashingformerAll, self).__init__()
...
#         self.bit=config.bit            
#         self.hashing_linear  = nn.Linear(self.config.hidden_size,self.bit , bias=True)


#     def init_code_embedding(self, em):
#         self.bert.code_embeddings.weight.data = em.clone()

#     def forward(self, input_ids,token_type_ids=None, attention_mask=None,position_ids=None, head_mask=None, img_feats=None,modal="l"):
#         text_len = input_ids.shape[1]//2
#         if(modal =="i"):
#             bert_outputs = self.bert(input_ids[:,text_len:], position_ids=position_ids, token_type_ids=token_type_ids[:,text_len:],
...
#         pooled_output = torch.mean(bert_outputs,1) 

#         hashing_code = self.hashing_linear(pooled_output)
#         hashing_code = torch.tanh(hashing_code)
#         return hashing_code

class Hashingformer(nn.Module):
    """
    Modified from BertForSequenceClassification to support oscar training.
    """
    def __init__(self,oscar_checkpoint_path,config):
        super(Hashingformer, self).__init__()
        self.class_number = config.class_number
        self.config = config
        if(oscar_checkpoint_path!=None and oscar_checkpoint_path!=""):
            self.bert = BertImgModel.from_pretrained(oscar_checkpoint_path, from_tf=bool('.ckpt' in oscar_checkpoint_path), config=config)
        ...
        new_config.vocab_size = config.class_number+1
        label_hidden_size = 768
        new_config.hidden_size = label_hidden_size
        self.labelnet = LabelNet(new_config)
        self.bit=config.bit            
        self.hashing_linear  = nn.Linear(self.config.hidden_size,self.bit , bias=True)
        self.label_hashing_linear  = nn.Linear(label_hidden_size,self.bit , bias=True)

    def init_code_embedding(self, em):
        self.bert.code_embeddings.weight.data = em.clone()

    def forward(self, input_ids,token_type_ids=None, attention_mask=None,position_ids=None, head_mask=None, img_feats=None,modal="i"):
        text_len = input_ids.shape[1]//2
        if(modal =="i"):
            bert_outputs = self.bert(input_ids[:,text_len:], position_ids=position_ids, token_type_ids=token_type_ids[:,text_len:],
                            attention_mask=attention_mask[:,text_len:], head_mask=head_mask, img_feats=img_feats) 
            ...
            bert_outputs = self.bert(input_ids[:,:text_len], position_ids=position_ids, token_type_ids=token_type_ids[:,:text_len],
                    attention_mask=attention_mask[:,:text_len], head_mask=head_mask, img_feats=None)
        pooled_output = torch.mean(bert_outputs[0],1)   
        hashing_code = self.hashing_linear(pooled_output)
        hashing_code = torch.tanh(hashing_code)
        return hashing_code

    def generate_label_code(self,label):
        label_out = self.labelnet(label)
        label_out = torch.mean(label_out[0],1)
        label_out = self.hashing_linear(label_out)
        hashing_code = torch.tanh(label_out)
        return hashing_code


class HashingformerUnit(nn.Module):
    """
    Modified from BertForSequenceClassification to support oscar training.
    """
    def __init__(self,oscar_checkpoint_path,config):
        super(HashingformerUnit, self).__init__()
        self.class_number = config.class_number
        self.config = config
        if(oscar_checkpoint_path!=None and oscar_checkpoint_path!=""):
            self.bert = BertImgModel.from_pretrained(oscar_checkpoint_path, from_tf=bool('.ckpt' in oscar_checkpoint_path), config=config)
            self.label_bert = BertImgModel.from_pretrained(oscar_checkpoint_path, from_tf=bool('.ckpt' in oscar_checkpoint_path), config=config)
        else:
            self.bert = BertImgModel(config)
            self.label_bert = BertImgModel(config)

        self.bit=config.bit            
        self.hashing_linear  = nn.Linear(self.config.hidden_size,self.bit , bias=True)
        self.final_hashing  = nn.Linear(self.config.hidden_size,self.bit , bias=True)
    ...
    def forward(self, input_ids,token_type_ids=None, attention_mask=None,position_ids=None, head_mask=None, img_feats=None,modal="l"):
        ...
        if(modal =="i"):
            ...
            bert_outputs = self.label_bert(input_ids)
            bert_outputs = bert_outputs[0]
            pooled_output = torch.mean(bert_outputs,1) 
        ...
        hashing_code = self.final_hashing(pooled_output)
        hashing_code = torch.tanh(hashing_code)
        return hashing_code
...
class HashingformerALL(nn.Module):
    ...
    def __init__(self,oscar_checkpoint_path,config):
        ...
        self.bit=config.bit            
        self.final_hashing  = nn.Linear(self.config.hidden_size,self.bit , bias=True)
        #self.classify_layer  = nn.Linear(self.config.hidden_size,self.class_number , bias=True)
    def init_code_embedding(self, em):
        self.bert.code_embeddings.weight.data = em.clone()
    ...
    def forward(self, input_ids,token_type_ids=None, attention_mask=None,position_ids=None, head_mask=None, img_feats=None,modal="l"):
        ...
        return hashing_code
class HashingformerALLNew(nn.Module):
    """
    Modified from BertForSequenceClassification to support oscar training.
    """
    def __init__(self,oscar_checkpoint_path,config):
        super(HashingformerALLNew, self).__init__()
        self.class_number = config.class_number
        self.config = config
        if(oscar_checkpoint_path!=None and oscar_checkpoint_path!=""):
            self.bert = BertImgModel.from_pretrained(oscar_checkpoint_path, from_tf=bool('.ckpt' in oscar_checkpoint_path), config=config)
    ...
    def forward(self, input_ids,token_type_ids=None, attention_mask=None,position_ids=None, head_mask=None, img_feats=None,modal="l"):
        text_len = self.config.max_seq_length_a
        if(modal =="i"):
            bert_outputs = self.bert(input_ids[:,text_len:], position_ids=position_ids, token_type_ids=token_type_ids[:,text_len:],
                            attention_mask=attention_mask[:,text_len:], head_mask=head_mask, img_feats=img_feats)
...
class HashingformerSingle(nn.Module):
    """
    Modified from BertForSequenceClassification to support oscar training.
    """
    def __init__(self,oscar_checkpoint_path,config):
        super(HashingformerSingle, self).__init__()
        self.class_number = config.class_number
        self.config = config
        if(oscar_checkpoint_path!=None and oscar_checkpoint_path!=""):
            self.bert = BertImgModel.from_pretrained(oscar_checkpoint_path, from_tf=bool('.ckpt' in oscar_checkpoint_path), config=config)
            self.bert1 = BertImgModel.from_pretrained(oscar_checkpoint_path, from_tf=bool('.ckpt' in oscar_checkpoint_path), config=config)
        else:
            self.bert = BertImgModel(config)
            self.bert1 = BertImgModel(config)
        self.bit=config.bit            
        self.final_hashing  = nn.Linear(self.config.hidden_size,self.bit , bias=True)
        #self.classify_layer  = nn.Linear(self.config.hidden_size,self.class_number , bias=True)
    def init_code_embedding(self, em):
    ...
    def forward(self, input_ids,token_type_ids=None, attention_mask=None,position_ids=None, head_mask=None, img_feats=None,modal="l"):
        ...
        if(modal =="i"):
            ...
            bert_outputs = self.bert1(input_ids[:,:text_len], position_ids=position_ids, token_type_ids=token_type_ids[:,:text_len],
                    attention_mask=attention_mask[:,:text_len], head_mask=head_mask, img_feats=None)
        ...
        return hashing_code
class HashingformerClass(nn.Module):
    """
    Modified from BertForSequenceClassification to support oscar training.
    """
    def __init__(self,oscar_checkpoint_path,config):
        super(HashingformerClass, self).__init__()
        self.class_number = config.class_number
        self.config = config
        if(oscar_checkpoint_path!=None and oscar_checkpoint_path!=""):
            self.bert = BertImgModel.from_pretrained(oscar_checkpoint_path, from_tf=bool('.ckpt' in oscar_checkpoint_path), config=config)
    ...
    def forward(self, input_ids,token_type_ids=None, attention_mask=None,position_ids=None, head_mask=None, img_feats=None,modal="i",o=False):
        text_len = input_ids.shape[1]//2
        if(modal =="i"):
            bert_outputs = self.bert(input_ids[:,text_len:], position_ids=position_ids, token_type_ids=token_type_ids[:,text_len:],
                            attention_mask=attention_mask[:,text_len:], head_mask=head_mask, img_feats=img_feats)                           
        ...
        if(not o):
            return hashing_code
        class_out = self.classify_layer(pooled_output)
        return hashing_code,torch.sigmoid(class_out)


class HashingformerALLPretrain(nn.Module):
    """
    Modified from BertForSequenceClassification to support oscar training.
    """
    def __init__(self,oscar_checkpoint_path,config):
        super(HashingformerALLPretrain, self).__init__()
        self.class_number = config.class_number
        self.config = config
        if(oscar_checkpoint_path!=None and oscar_checkpoint_path!=""):
            self.bert = BertImgModel.from_pretrained(oscar_checkpoint_path, from_tf=bool('.ckpt' in oscar_checkpoint_path), config=config)
    ...
    def forward(self, input_ids,token_type_ids=None, attention_mask=None,position_ids=None, head_mask=None, img_feats=None,modal="l",classify=False):
        text_len = input_ids.shape[1]//2
        if(modal =="i"):
            bert_outputs = self.bert(input_ids[:,text_len:], position_ids=position_ids, token_type_ids=token_type_ids[:,text_len:],
                            attention_mask=attention_mask[:,text_len:], head_mask=head_mask, img_feats=img_feats)
