...
class ContrastiveTwins(nn.Module):
    ...
    @autocast()
    def forward(self, input_ids,token_type_ids=None, attention_mask=None,position_ids=None, head_mask=None, img_feats=None):#label 8
...
class ContrastiveSingle(nn.Module):
    """
    Modified from BertForSequenceClassification to support oscar training.
    """
    def __init__(self,oscar_checkpoint_path,config):
        super(ContrastiveSingle, self).__init__()
        self.num_labels = config.num_labels
        self.config = config
        if(oscar_checkpoint_path!=None and oscar_checkpoint_path!=""):
            self.bert = BertImgModel.from_pretrained(oscar_checkpoint_path, from_tf=bool('.ckpt' in oscar_checkpoint_path), config=config)
            #self.text_bert = BertImgModel.from_pretrained(oscar_checkpoint_path, from_tf=bool('.ckpt' in oscar_checkpoint_path), config=config)
        else:
            self.bert = BertImgModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.relu = nn.ReLU()
    ...
    def init_code_embedding(self, em):
        self.bert.code_embeddings.weight.data = em.clone()
    @autocast()
    def forward(self, input_ids,token_type_ids=None, attention_mask=None,position_ids=None, head_mask=None, img_feats=None,modal = "I"):#label 8
        if(modal=="I"):
            return self.i_forwards(input_ids=input_ids,token_type_ids=token_type_ids, attention_mask=attention_mask,position_ids=position_ids, head_mask=head_mask, img_feats=img_feats)
        elif(modal=="T"):
            return self.t_forwards(input_ids=input_ids,token_type_ids=token_type_ids, attention_mask=attention_mask,position_ids=position_ids, head_mask=head_mask, img_feats=img_feats)
        return None
    ...
    def i_forwards(self, input_ids,token_type_ids=None, attention_mask=None,position_ids=None, head_mask=None, img_feats=None):
        ...
        i_feature =torch.tanh(i_feature)
        return i_feature  
class ContrastiveTwinsAttention(nn.Module):
    """
    Modified from BertForSequenceClassification to support oscar training.
    """
    def __init__(self,oscar_checkpoint_path,config):
        super(ContrastiveTwinsAttention, self).__init__()
        self.num_labels = config.num_labels
        self.config = config
        if(oscar_checkpoint_path!=None and oscar_checkpoint_path!=""):
            self.bert = BertImgModel.from_pretrained(oscar_checkpoint_path, from_tf=bool('.ckpt' in oscar_checkpoint_path), config=config)
    ...
    def forward(self, input_ids,token_type_ids=None, attention_mask=None,position_ids=None, head_mask=None, img_feats=None):#label 8
        ...
        all_t_feature = torch.tanh(self.dropout(outputs_text[0])) 
        all_i_feature = torch.tanh(self.dropout(outputs_img[0])) 

        outputs = (i_feature,t_feature, all_i_feature, all_t_feature)
